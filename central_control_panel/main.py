#!/usr/bin/env python3
"""
stretch_study_controller.py  (ROS 2)

One-file “central control panel + study orchestrator” for your Stretch study:
- Finite-state machine that walks through:
  Intro -> Global settings wizard -> Follow/walk -> Desk config -> Desk demo (optional)
  -> Bed config -> Bed demo (optional) -> Kitchen config -> Kitchen demo (optional)
  -> Situational rules -> Round 2 profile switch -> repeat -> End
- Profiles: "user" (Round 1) and "jackie" (Round 2)
- Settings storage + timestamped archiving to JSON
- Terminal hotkeys for RA override (advance, follow toggle, station set, demo run, estop)
...
- “Follow me” placeholder that creeps forward (so the flow works immediately)
  You can later replace it with true follow by updating self.follow_target in on_follow_target()
...
UI event format (publish JSON to /study_event):
  ...
  # set room setting (room: desk|bed|kitchen)
  {"type":"set","scope":"room","room":"desk","key":"cleaning_thoroughness","value":"twice"}

  # station arrival (voice or button)
  {"type":"arrive","room":"desk"}

  # confirm demo
  {"type":"demo_confirm","room":"desk","yes":true}

  # situational rules (store blob)
  {"type":"set","scope":"rules","value":{"no_go":[...],"time_rules":[...]}}

  # switch active profile (Round 2)
  {"type":"profile","value":"jackie"}

  # explicit advance (if you want UI to drive progression)
  {"type":"advance"}

Terminal hotkeys (RA / debugging):
  ENTER : advance script (when waiting)
  f     : toggle FOLLOW mode (walk together)
  1/2/3 : mark arrival at desk/bed/kitchen (same as {"type":"arrive"})
  d     : toggle demo confirm yes for current room (or run demo if already confirmed)
  s     : stop base (IDLE)
...
Notes:
- This script publishes “speech” lines to a ROS topic /stretch_say (std_msgs/String) AND prints them.
  You can bridge /stretch_say to your real TTS system however you like.
- Demos are currently SAFE PLACEHOLDERS (base gestures only). Replace with Stretch Body arm sequences later.
"""
...
class StudyState:
    ...
    demo_confirm: dict = field(default_factory=lambda: {"desk": None, "bed": None, "kitchen": None})
...
class Step:
    step_id: str
    prompt: str
    scope: str                 # "global" or "room" or "rules"
    key: str | None = None     # setting key (global/room)
    room: str | None = None    # for room steps
    valid: callable | None = None  # validate function
...
class StretchStudyController(Node):
    def __init__(self):
        ...
        self.log_info(f"Publishing cmd_vel to: {self.cmd_vel_topic}")
        self.log_info(f"Listening for UI events on: {self.event_topic}")
        self.log_info(f"Publishing speech lines on: {self.say_topic}")
        self.say("System ready. Press ENTER to begin, or send {\"type\":\"advance\"} on /study_event.")
    ...
    def say(self, text: str):
        # Publish to topic and print
        self.log_info(f"[SAY] {text}")
    ...
    def save_state(self):
        ...
        try:
            ...
            self.log_warn(f"Failed to save state: {e}")

    def archive_snapshot(self, label: str):
        with self._lock:
            ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            path = os.path.join(self.archive_dir, f"{ts}_{label}.json")
            try:
        ...
        try:
            ...
            self.log_info(f"Archived snapshot: {path}")
        except Exception as e:
            self.log_warn(f"Failed to archive snapshot: {e}")
    ...
    def effective_settings(self, room: str | None = None) -> dict:
        pd = self.profile()
        eff = dict(pd.global_defaults)
        if room:
            eff.update(pd.rooms.get(room, {}))
        # Apply time rules could go here (optional)
        return eff
    ...
    def control_tick(self):
        # Always enforce ESTOP
        if self.estop:
            self.publish_stop()
            return

        # Follow placeholder mode (creep forward)
        if self.mode_follow and self.phase in (Phase.WALK_AND_STATIONS, Phase.STATION_CONFIG):
            ...
            max_v = self.follow_max_v_by_speed.get(eff.get("movement_speed", "medium"), 0.25)
    ...
    def on_event_msg(self, msg: String):
        try:
            ...
            self.log_warn(f"Bad JSON on /{self.event_topic}: {e}")
            return
    ...
    def handle_event(self, evt: dict):
        ...
        if etype == "profile":
            ...
            if val in self.profiles:
                ...
                self.say(f"Profile set to {val}.")
                self.archive_snapshot(f"profile_{val}")
            else:
        ...
        if etype == "demo_confirm":
            ...
            if room in ("desk", "bed", "kitchen"):
                ...
                if yes:
                    self.say(f"Okay. I will perform the {room} demo.")
                else:
                    self.say(f"Okay. I will skip the {room} demo.")
                # If we are currently prompting for demo, advance automatically
        ...
        if etype == "set":
            scope = evt.get("scope")
            if scope == "global":
                self.apply_setting_global(evt.get("key"), evt.get("value"))
                self.maybe_confirm()
                self.advance_if_waiting_for(evt.get("key"))
                return

            if scope == "room":
                room = evt.get("room", self.study.current_room)
                self.apply_setting_room(room, evt.get("key"), evt.get("value"))
                self.maybe_confirm()
                self.advance_if_waiting_for(f"{room}.{evt.get('key')}")
                return
            ...
            if scope == "rules":
                ...
                if isinstance(val, dict):
                    ...
                    self.archive_snapshot(f"rules_{self.study.active_profile}")
                    self.advance_if_waiting_for("rules")
                return

        self.log_warn(f"Unhandled event: {evt}")

    def maybe_confirm(self):
        eff = self.effective_settings(None)
        if eff.get("confirmation", True):
            self.say("Confirmed.")

    def advance_if_waiting_for(self, token: str):
        # If we are waiting for a specific key, and this matches, advance.
        if self.study.waiting_for == token:
            self.advance()

    # -------- Apply settings --------

    def apply_setting_global(self, key: str, value):
        if not key:
            return
        pd = self.profile()
        pd.global_defaults[key] = value
        self.log_info(f"Set GLOBAL {key}={value} (profile={self.study.active_profile})")
        self.archive_snapshot(f"global_{self.study.active_profile}")

    def apply_setting_room(self, room: str, key: str, value):
        if not room or not key:
            return
        pd = self.profile()
        pd.rooms.setdefault(room, {})
        pd.rooms[room][key] = value
        self.log_info(f"Set ROOM[{room}] {key}={value} (profile={self.study.active_profile})")
        self.archive_snapshot(f"room_{room}_{self.study.active_profile}")
    ...
    def enter_intro(self):
        ...
        self.study.waiting_for = ""  # ENTER drives these
    ...
    def enter_global_wizard(self):
        ...
        self.pending_steps = [
            ...
            Step(
                ...
                scope="global",
                key="confirmation",
                valid=lambda v: isinstance(v, bool) or v in ("yes", "no", "true", "false", 0, 1),
            ),
            Step(
    ...
    def enter_walk(self):
        ...
        self.say("You can press 'f' to toggle follow mode. Press 1/2/3 to mark arrival at desk/bed/kitchen.")
    ...
    def enter_station_config(self, room: str):
        ...
        # Room-specific steps + demo setting
        demo_key = {
            "desk": "cleaning_thoroughness",
            "bed": "pillow_arrangement",
            "kitchen": "snack_preference",
        }[room]
        ...
        self.pending_steps = [
            Step(
                step_id=f"{room}_movement_speed",
                prompt=f"In the {room} area, please set movement speed: slow, medium, or fast.",
                scope="room", room=room, key="movement_speed",
                valid=lambda v: v in ("slow", "medium", "fast"),
            ),
            Step(
                step_id=f"{room}_voice_volume",
                prompt=f"In the {room} area, please set voice volume (0 to 100).",
                scope="room", room=room, key="voice_volume",
                valid=lambda v: isinstance(v, (int, float)) and 0 <= int(v) <= 100,
            ),
            Step(
                step_id=f"{room}_explainability",
                prompt=f"In the {room} area, please set explainability: none, short, or full.",
                scope="room", room=room, key="explainability",
                valid=lambda v: v in ("none", "short", "full"),
            ),
            Step(
                step_id=f"{room}_social_distance",
                prompt=f"In the {room} area, please set social distance: close, medium, or far.",
                scope="room", room=room, key="social_distance",
                valid=lambda v: v in ("close", "medium", "far"),
            ),
            Step(
                step_id=f"{room}_{demo_key}",
                prompt=demo_prompt,
        ...
        self.say(f"Affirmative — I have registered this area as the {room}.")
    ...
    def enter_demo_prompt(self, room: str):
        ...
        self.step_index = 0
        self.pending_steps = []
        self.study.waiting_for = ""  # waiting on demo_confirm event or key
        self.say(f"I have received your {room} customizations. Would you like me to perform the {room} demo?")
        self.say("Send {\"type\":\"demo_confirm\",\"room\":\"%s\",\"yes\":true/false} or press 'd' to confirm yes." % room)

    def enter_demo_run(self, room: str):
        self.phase = Phase.STATION_DEMO_RUN
        self.step_index = 0
        self.pending_steps = []
        self.say(f"Performing {room} demo now.")
        # Stop follow and base
        self.mode_follow = False
        self.publish_stop()

        # Placeholder “safe demo”: base gesture + narration
        if room == "desk":
            passes = self.effective_settings("desk").get("cleaning_thoroughness", "once")
            self.say(f"Here is how I will wipe your desk: {passes}. (Demo placeholder)")
            self.base_gesture_wiggle()
        elif room == "bed":
            arrangement = self.effective_settings("bed").get("pillow_arrangement", "center")
            self.say(f"Here is how I will arrange the pillow: {arrangement}. (Demo placeholder)")
            self.base_gesture_wiggle()
        elif room == "kitchen":
            snack = self.effective_settings("kitchen").get("snack_preference", "cheetos")
            self.say(f"Here is the snack I will retrieve: {snack}. (Demo placeholder)")
            self.base_gesture_wiggle()

        self.say("Demo complete.")
        # After demo, return to walk mode to reach next station
        self.enter_walk()
    ...
    def enter_rules(self):
        ...
        self.pending_steps = [
            Step(
                step_id="rules",
                prompt=("Before we end this round, you may set home-wide situational settings, "
                        "like no-go zones and time-based rules. Please submit your situational rules now."),
                scope="rules",
                key=None,
        ...
        self.pending_steps = [
            Step(
                ...
                prompt=("Welcome back. I understand Jackie will be staying with you for a week. "
                        "We’ll now walk through the apartment again. This time, please configure my behavior "
                        "to best support Jackie’s needs."),
                scope="meta",
            )
        ]
        self.present_current_step()
    ...
    def present_current_step(self):
        if self.step_index >= len(self.pending_steps):
            # No steps to show; rely on advance() transitions
            return

        step = self.pending_steps[self.step_index]
        self.say(step.prompt)

        # Set waiting token (what event unblocks us)
        if step.scope == "global" and step.key:
            self.study.waiting_for = step.key
            self.say(f"(Waiting for global setting '{step.key}' via /{self.event_topic})")

        elif step.scope == "room" and step.room and step.key:
            self.study.waiting_for = f"{step.room}.{step.key}"
            self.say(f"(Waiting for room setting '{step.room}.{step.key}' via /{self.event_topic})")

        elif step.scope == "rules":
            self.study.waiting_for = "rules"
            self.say(f"(Waiting for situational rules via /{self.event_topic})")
    ...
    def advance(self):
        """
        Main "next" button of the whole system.
        ENTER key or {"type":"advance"} calls this.
        """
        with self._lock:
            # If we are inside a step list, advance steps first
            if self.step_index < len(self.pending_steps):
                # If we are waiting for an external setting, don't skip unless it's a meta step
                step = self.pending_steps[self.step_index]
                if step.scope in ("global", "room", "rules") and self.study.waiting_for:
                    # Don’t allow skipping required inputs accidentally (RA can press ENTER twice if needed)
                    self.say("Still waiting for input for this step. (You can still force skip by pressing ENTER again.)")
                    # Force-skip on double-enter: clear waiting_for on second call quickly
                    # Simple heuristic: if waiting_for was already announced recently, allow skip
                    # We'll implement: if waiting_for exists, clear it and allow advancing next time.
                    self.study.waiting_for = ""
                    return
                ...
                if self.step_index < len(self.pending_steps):
                    self.present_current_step()
                    return
                # Step list finished; fall through to phase transition

            # Phase transitions
            if self.phase == Phase.INTRO:
                # After intro steps complete
                self.enter_global_wizard()
                return

            if self.phase == Phase.GLOBAL_WIZARD:
                # Finished global wizard -> walk
                self.archive_snapshot(f"global_done_{self.study.active_profile}")
                self.enter_walk()
                return

            if self.phase == Phase.STATION_CONFIG:
                # Finished room config -> demo prompt
                room = self.study.current_room
                self.archive_snapshot(f"{room}_config_done_{self.study.active_profile}")
            ...
            if self.phase == Phase.RULES:
                ...
                if self.study.active_profile == "user":
                    ...
                    # Switch to Jackie profile (Round 2)
            ...
            if self.phase == Phase.WALK_AND_STATIONS:
                self.say("Waiting for you to arrive at a station (desk, bed, kitchen).")
                return

            if self.phase == Phase.STATION_DEMO_RUN:
                return
    ...
    def on_arrive(self, room: str):
        # Stop walking/follow on arrival
        self.mode_follow = False
        self.publish_stop()

        # Decide where we are in the script
        if self.phase == Phase.WALK_AND_STATIONS:
            # Start the correct station config
            self.enter_station_config(room)
            return

        self.say(f"Arrival at {room} received, but I'm not currently walking between stations.")
    ...
    def base_gesture_wiggle(self):
        """Safe, low-amplitude wiggle: tiny left-right rotation."""
        if self.estop:
            return

        def _run():
            # small angular motions
    ...
    def print_effective(self):
        ...
        self.log_info(f"Active profile: {self.study.active_profile}")
        self.log_info(f"Current room: {self.study.current_room}")
        self.log_info(f"Global defaults: {json.dumps(eff_global, indent=2)}")
        self.log_info(f"Effective in room: {json.dumps(eff_room, indent=2)}")


# ----------------------------
# Terminal key loop (single-char, no Enter needed except for ENTER)
# ----------------------------

def terminal_key_loop(node: StretchStudyController):
    """
    Raw single-key input without curses (works over SSH).
    """
    import termios
    import tty
    ...
    node.log_info("Terminal hotkeys ready: ENTER advance | f follow | 1/2/3 arrive | d demo yes | e estop | p print | s stop | q quit")
    ...
    try:
        while rclpy.ok():
            ...
            if ch == "d":
                ...
                # If we're prompting, interpret as YES
                node.handle_event({"type": "demo_confirm", "room": room, "yes": True})
                # If not prompting, run demo anyway (debug)
